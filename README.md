...
new "high flow" 0.40 mm nozzle came with my MK3.9S upgrade it. Nothing I did in the slicer got the test cube wall thicknesses anywhere closer 
than 10% too thick.
...
the flattened edge. 
I bought these from Amazon and cut them to length: https://www.amazon.com/dp/B0C274W667.
The length is enough to hold the 20 tooth gearbox pulley. The STL part is from OnStepGearBoxElevationPully.scad. 
It's the same part on the azimuth gearbox as well. The inner carrier prints with a D-profile 6mm hole that should have a cut-to-length piece of steel shaft
...
required. They can be set screws of at least 4mm length, but there is alternatively room for a longer socket head
...
M3 x 8mm socket head screws and mating M3 hex nuts. (A longer M3 screw is useful to first pull the nut into the well of the housing.)
...
this far into this project to consider using a more off-the-shelf electronics design. I bought an assembled OnStepX 
capable MaxPCB4E from https://graydigitalarts.com/product/maxpcb4e/.  I did not install the web server 
Teensy that
board supports because I only want to control my telescope from the OnStep Android app via 
Bluetooth. https://play.google.com/store/search?q=OnStep.
I bought this HC-06 bluetooth-to-serial 
device, https://www.amazon.com/dp/B07VL6ZH67, and jumpered the necessary four wires on the MaxPCB4E to it. 
...
as each cell-less position is occupied by a dummy D cell (a D-cell sized device that presents a short circuit.)
There exist 14-packs of alkaline D cells for sale, which means the booster makes it practical to use those 14 in two
batches of 7.
With my particular stepper motor amperage requirements (about 1.5A each), the internal resistance of even brand new
D alkaline cells makes six cells the fewest that will deliver the necessary amperage for that chemistry. The
boost converter also makes the system less sensitive to the battery chemistry such that, for example,
rechargeable D cells will work, and maybe fewer than eight if the builder prefers. Of course,
using fewer cells in the battery also means a shorter battery life.
...
OnStep supports limit switch input and I designed this little part, LimitSwtichBracket.FCStd, but I have not installed,
nor wired it up because I don't mind watching for a problem as the telescope moves, and,
...
roughly evaluate wheter, after a "Reset to Home" in the Initialize screen, that I could use "Guide/Focus" to
move to any elevation and/or azimuth, then return to the Initialize screen to command "Return to Home" and 
confirmed through the finder that the motors in fact move it back to home. After this, I connected my ST4
